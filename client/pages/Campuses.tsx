...
export default function Campuses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data, error } = await supabase.from("campuses").select("id,name,code,city,address,status,created_at").order("created_at", { ascending: false });
        if (!error && Array.isArray(data)) {
          const mapped: Campus[] = data.map((c: any) => ({ id: String(c.id), name: c.name, code: c.code, city: c.city, address: c.address || "", status: (c.status as CampusStatus) || "active" }));
          setCampuses(mapped);
      ...
      try {
        ...
        if (ch) {
          ch.on("postgres_changes", { event: "*", schema: "public", table: "campuses" }, (payload: any) => {
            const rec = payload.new || payload.old;
            if (!rec) return;
            const item: Campus = { id: String(rec.id), name: rec.name, code: rec.code, city: rec.city, address: rec.address || "", status: (rec.status as CampusStatus) || "active" };
            setCampuses((prev) => {
  ...
  const addCampus = async (c: Omit<Campus, "id">) => {
    try {
      const { data, error } = await supabase
        .from("campuses")
        .insert([{ name: c.name, code: c.code, city: c.city, address: c.address || null, status: c.status }])
        .select("id,name,code,city,address,status")
        .single();
      if (error) throw error;
      const id = String((data as any).id);
      setCampuses((prev) => [{ ...c, id }, ...prev]);
      toast({ title: "Campus added", description: `${c.name} (${c.code}) created.` });
    } catch (e: any) {
      toast({ title: "Failed to add campus", description: e?.message || "Check Supabase policies" });
    }
  };
  ...
  const updateCampus = async (id: string, patch: Partial<Campus>) => {
    try {
      ...
      const { error } = await supabase.from("campuses").update(update).eq("id", id);
      if (error) throw error;
      setCampuses((prev) => prev.map((c) => (c.id === id ? { ...c, ...patch } : c)));
      toast({ title: "Campus updated" });
    } catch (e: any) {
      toast({ title: "Update failed", description: e?.message || "Check policies" });
    }
  };

  const setStatus = (id: string, status: CampusStatus) => updateCampus(id, { status });
